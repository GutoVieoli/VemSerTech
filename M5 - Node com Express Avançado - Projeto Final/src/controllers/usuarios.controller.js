...
const deleteUser = async (requisicao, resposta) => {
    const email = requisicao.body.email;
    const senha = requisicao.body.senha;
        ...
        if (procuraConta) {                                                   // Tenta Logar
            ...
            if( bcrypt.hashSync(senha, salt) === senhaBD){                   // Senha aceita
                ...
                try {
                    ...
                    resposta.status(200).send({ message: 'Usuario excluído com sucesso!' });                
                }
                catch(error) {
                    console.error('Erro ao excluir produto:', error);
                    resposta.status(500).send({ message: 'Ocorreu algum erro inesperado ao tentar excluir!' });                
                }

            } 
            else {                                                           // Senha incorreta
                resposta.status(400).send({message: 'Senha incorreta. Nao foi possivel deletar o usuario'})
            }
        } 
        else {
            resposta.status(400).send({message: "Email não encontrado. Nao foi possivel deletar o usuario"})
        }
    }
    else 
    {
